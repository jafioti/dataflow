...
fn tokenize_alphabet() {
    let letters: Vec<String> = vec!["h", "e", "l", "l", "o"]
        .iter()
        .map(|t| (*t).to_string())
        .collect();
...
#[test]
fn tokenize_spaces() {
    let tokens: Vec<String> = vec!["hello", "how", "are", "you"]
        .iter()
        .map(|t| (*t).to_string())
        .collect();
...
fn tokenize_sentences() {
    let tokens: Vec<String> = vec!["hello, how are you?", "good, how are you?"]
        .iter()
        .map(|t| (*t).to_string())
        .collect();
    let tokenizer = SentenceTokenizer::load();
    assert_eq!(
        tokenizer.tokenize("hello, how are you? good, how are you?"),
        tokens
    );
}
...
fn tokenize_bpe() {
    ...
    assert_eq!(
        tokenizer.batch_tokenize(vec!["hello, how are you".to_string()]),
        vec![tokens.clone()]
    );
    assert_eq!(tokenizer.tokenize("hello, how are you"), tokens);
}
